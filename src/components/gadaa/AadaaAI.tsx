...
import { aiPrompts, aiReply } from "@/lib/gadaa-data";
...
export function AadaaPanel({ compact = false }: { compact?: boolean }) {
  const [messages, setMessages] = useState<Msg[]>([
    {
      role: "ai",
      text: "Akkam! I am Aadaa AI, your 24/7 Oromia cultural guide. Ask me about festivals, Afan Oromo phrases, or where to travel next.",
    },
  ]);
  ...
  function send(text: string) {
    ...
    setInput("");
    setTimeout(
      () => setMessages((m) => [...m, { role: "ai", text: aiReply(q) }]),
      380,
    );
  }
      ...
      <div className="flex items-center gap-3 border-b border-border px-4 py-3">
        ...
        <div className="min-w-0">
          ...
          <p className="truncate text-[11px] text-muted-foreground">
            24/7 cultural &amp; language guide
          </p>
        </div>
      </div>

      <div
        className={`flex-1 space-y-3 overflow-y-auto px-4 py-4 ${compact ? "max-h-80" : ""}`}
      >
        {messages.map((m, i) => (
          <div
            key={i}
...
export function AadaaWidget() {
  ...
  return (
    <>
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        aria-label="Open Aadaa AI guide"
